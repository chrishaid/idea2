...
renderValueBox({
  valueBox(value = contacts_filtered()$begin,
           caption = sprintf("Contacts since Apr 8th (goal: %s)",
...
fillRow(flex = c(.6,.4),
        ...
        DT::dataTableOutput("unicontacts_table")
)

data_unique_contacts <- reactive({
  ...
  p <- contacts_details %>%
   filter_(~kipp_hs_class_c %in% classes,
           ~status_c %in% statuses,
           filter_cond) %>%
   tidyr::unite(col = first_last,
                first_name.x,
                last_name.x,
                sep = " ",
                remove=TRUE) %>%
  ...
  return(
    bind_rows(first_success, first_outreach, outreach_success) %>%
      arrange(date_c)
  )
})

output$unicontacts_table <- DT::renderDataTable({

  validate(
    need(input$plot_brush != "",
         "Brush across a graph to display contact details.")
  )
  if(!grepl("~", input$facet_by)) {

  if(input$facet_by == "name.y") { #if counselor
    filter_cond <- ~ name.y %in% input$plot_brush$panelvar1
    select_col <- ~ Class

  } else { # if class
    filter_cond <- ~ kipp_hs_class_c %in% input$plot_brush$panelvar1
    select_col <- ~ Counselor
  }

  dt_unique <- data_unique_contacts() %>%
    filter_(filter_cond,
            ~ date_c >= input$plot_brush$xmin,
            ~ date_c <= input$plot_brush$xmax) %>%
    rename(Name = first_last,
           Class = kipp_hs_class_c,
           Counselor = name.y,
           Status = status_c) %>%
    mutate(Date = as.character(as_date(as_datetime(date_c)))) %>%
    select_(~ Date,
            ~ Name,
            select_col,
            ~ Status)
   ...
   dt_unique <-  data_unique_contacts() %>%
    filter_(~ date_c >= input$plot_brush$xmin,
            ~ date_c <= input$plot_brush$xmax) %>%
    mutate(Date = as.character(as_date(as_datetime(date_c)))) %>%
    select(Date,
           Name = first_last,
...
if(!grepl("~", input$facet_by)) {
  ...
  u_table <- data_unique_contacts() %>%
    ...
    facet_wrap(input$facet_by, ncol = 1) +
...
fillRow(flex = c(.7,.3),
        plotOutput("contacts_flow",
                  brush = brushOpts(id = "total_brush",
                                    direction = "x")
   ),
   #verbatimTextOutput("test3")
       DT::dataTableOutput("table_c_flow")
)
output$test3 <- renderPrint({
  input$total_brush
  })
output$table_c_flow <- DT::renderDataTable({
  validate(
    need(input$total_brush != "", "Brush across a graph to display contact totals.")
  )
  if(!grepl("~", input$facet_by)) { # not class and counselor

    if(input$facet_by == "name.y") { #if counselor
      filter_cond <- ~ name.y %in% input$total_brush$panelvar1
      group_c <- "kipp_hs_class_c"
      row_c <- "Class"
    } else { # if class
      filter_cond <- ~ kipp_hs_class_c %in% input$total_brush$panelvar1
      group_c <- "name.y"
      row_c <- "Counselor"
    }
  ...
  c <- contacts_details %>%
    ...
    select(date_c,
           ...
           name.y,
           status_c) %>%
    filter_(~ date_c >= input$total_brush$xmin,
            ~ date_c <= input$total_brush$xmax,
            filter_cond)

  u_successful <- c %>%
    filter(status_c %in% "Successful") %>%
    group_by_(group_c) %>%
    distinct(contact_c, .keep_all = TRUE) %>%
    #group_by(name.y) %>%
    #filter(name.y %in% input$total_brush$panelvar1) %>%
    summarise(N=n()) %>%
    select_(group_c,
            'Unique Successful' = ~ N)

  u_outreach <-  c %>%
    filter(status_c %in% "Outreach") %>%
    group_by_(group_c) %>%
    distinct(contact_c, .keep_all = TRUE) %>%
    #group_by(name.y) %>%
    #filter(name.y %in% input$total_brush$panelvar1) %>%
  ...
  c_fin <-  c %>%
    ...
    group_by_(#date_c,
             group_c,
             ~ status_c) %>%
    summarise(N=n()) %>% # OR remove date_c group by to get totals
    tidyr:: spread(status_c, N, fill = "") %>%
    #rename(#Counselor = name.y,
    #       #Class = kipp_hs_class_c,
  #         'Total Successful' = Successful,
  #         'Total Outreach' = Outreach) %>%
    select_(group_c,
            'Total Successful' = ~ Successful,
            'Total Outreach' = ~ Outreach)

 total_table <- c_fin %>%
  left_join(u_successful, by = group_c) %>%
  left_join(u_outreach, by = group_c) %>%
  tidyr::replace_na(replace = list(`Unique Outreach` = "",
                                   `Unique Successful` = "")) %>%
  as.data.frame()

 trans_table <- t(total_table)

DT::datatable(trans_table[-1,] %>%
                as.data.frame(),
              colnames = as.vector(total_table[,1]),
...
DT::formatStyle(c(1:nrow(total_table)), 'text-align' = 'right')
  ...
  c <- contacts_details %>%
    ...
    select(date_c,
           ...
           name.y,
           status_c) %>%
    filter(date_c >= input$total_brush$xmin,
           date_c <= input$total_brush$xmax,
           name.y %in% input$total_brush$panelvar2,
           kipp_hs_class_c %in% input$total_brush$panelvar1)
...
c_fin <-  c %>%
  ...
  if(!"Outreach" %in% colnames(c_fin)){
      c_fin <- c_fin %>%
        mutate('Total Outreach' = "") %>%
        select(name.y,
               'Total Successful' = Successful,
               `Total Outreach`)
    } else {
...
total_table <- c_fin %>%
 left_join(u_successful, by = "name.y") %>%
 left_join(u_outreach, by = "name.y") %>%
 tidyr::replace_na(replace = list(`Unique Outreach` = "",
                                  `Unique Successful` = "")) %>%
 as.data.frame()
...
Summer Melt {data-navmenu="College" data-orientation=rows}
========================================================

Inputs{.sidebar}
----------------------------------------------------------
```{r}
selectInput("melt_class", "Class",
            choices = c(2014,
                        2015,
                        2016),
            selected = c(2016,
...
Row {.tabset .tabset-fade}
-----------------------------------------------------------------------
###Complete Melt {.no-padding}

```{r}
m_data <- reactive({
  if(length(input$melt_class) == 1){
    m_data <- melt_data(input$melt_class, denom_melt, enrolled_melt)
  } else if(length(input$melt_class) == 2) {
    c1 <- melt_data(input$melt_class[1], denom_melt, enrolled_melt)
    c2 <- melt_data(input$melt_class[2], denom_melt, enrolled_melt)
    m_data <- list()
    for(i in 1:6){
      m_data[[i]] <- bind_rows(c1[i], c2[i])
    }
    names(m_data) <- c("dm", "did_not_enroll",
                       "prop_melt", "melt_4_2",
                       "denom_4yr", "prop_melt_4_2")
  } else {
    c1 <- melt_data(input$melt_class[1], denom_melt, enrolled_melt)
    c2 <- melt_data(input$melt_class[2], denom_melt, enrolled_melt)
    c3 <- melt_data(input$melt_class[3], denom_melt, enrolled_melt)
    m_data <- list()
      for(i in 1:6){
        m_data[[i]] <- bind_rows(c1[i], c2[i], c3[i])
      }
      names(m_data) <- c("dm", "did_not_enroll",
                         "prop_melt", "melt_4_2",
                         "denom_4yr", "prop_melt_4_2")
...
output$complete_melt <- renderPlot({

 m_data()$dm %>%
 left_join(m_data()$did_not_enroll) %>%
 ...
 ggplot() +
  geom_bar(aes(x = class,
           ...
           color = "#255694",
           width = .5) +
  scale_fill_manual("", values = c("TRUE" = "#255694",
                                   "FALSE" = NA),
                    breaks = c("TRUE",
                               "FALSE"),
                    labels = c("Enrolled as of 10/01",
                               "Did Not Enroll")) +
  geom_text(data = m_data()$prop_melt %>%
              group_by(class) %>%
              mutate(display = max(N)) %>%
              filter(!enrolled),
            aes(x = class,
                y = display,
                label = sprintf("%s%%" ,prop)),
          color = "#255694",
          hjust = -.5,
          fontface = "bold") +
  geom_text(data = m_data()$prop_melt %>%
...
output$complete_melt_table <- renderTable({
  m_data()$did_not_enroll %>%
  filter(class %in% input$melt_class) %>%
  arrange(desc(as.numeric(class)), l_name) %>%
  tidyr::unite(Name, f_name, l_name, sep = " ") %>%
  select(Name,
         Class = class)
  })

fillRow(flex = c(.8, .2),
        plotOutput("complete_melt"),
        tableOutput("complete_melt_table")
        )
```

###4-year to 2-year Melt {.no-padding}

```{r}
output$melt_4_2 <- renderPlot({
  m_data()$denom_4yr %>%
  left_join(m_data()$melt_4_2,
            by = c("id",
                   "f_name",
                   "l_name",
                   "class")) %>%
  mutate(melt = ifelse(is.na(melt), FALSE, melt),
         melt = factor(melt, levels = c("TRUE", "FALSE"))) %>%
  ggplot() +
  geom_bar(aes(x= class,
              fill = melt),
           stat = "count",
           color = "#255694",
           width = .5) +
  geom_text(data = m_data()$prop_melt_4_2 %>%
              group_by(class) %>%
              mutate(display = max(N)) %>%
              filter(melt),
            aes(x = class,
  ...
  geom_text(data = m_data()$prop_melt_4_2 %>%
            ...
            fontface = "bold",
            hjust = 1) +
  scale_fill_manual("", values = c("FALSE" = "#255694",
                                   "TRUE" = NA),
                    breaks = c("FALSE",
                               "TRUE"),
                    labels = c("Enrollment: 4-year",
                               "Enrollment: 2-year")) +
  coord_flip() +
  theme_bw() +
  theme(legend.position = "bottom") +
  ...
  output$melt_4_2_table <- renderTable({
    m_data()$melt_4_2 %>%
    filter(class %in% input$melt_class) %>%
    arrange(desc(as.numeric(class)), l_name) %>%
    tidyr::unite(Name, f_name, l_name, sep = " ") %>%
