...
fillCol(flex = c(.7,.3),
       ...
       tableOutput("table_c_flow")
)

output$table_c_flow <- renderTable({
  validate(
  ...
  c_fin <-  c %>%
    ...
    tidyr:: spread(status_c, N) %>%
    rename(Counselor = name.y,
           'Total Successful' = Successful,
           'Total Outreach' = Outreach)

 total_table <- bind_cols(c_fin, u_outreach, u_successful)
 total_table
 })
...
m_data <- reactive({
  if(length(input$melt_class) == 1){
    ...
    names(m_data) <- c("dm", "did_not_enroll", "prop_melt", "melt_4_2")
  } else {
    c1 <- melt_data(input$melt_class[1], denom_melt, enrolled_melt)
    c2 <- melt_data(input$melt_class[2], denom_melt, enrolled_melt)
...
output$test_info <- renderPrint({input$c_melt_click})
fillRow(flex = c(.8, .2),
